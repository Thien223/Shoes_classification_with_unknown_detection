...
class EfficientNet(nn.Module):
    def __init__(self, num_classes=1000,
        ...
        # self.features = nn.Conv2d(out_channels, 5, 1, 1, groups=1, bias=False)
    ...
    def forward(self, x):
        ...
        fc_x = self.fc(x)
        # softmax_x = self.softmax(fc_x)
        #
        # n_samples = fc_x.shape[0]
        # c = torch.sum(fc_x, dim=0)
        # self.C = c / n_samples
        # # If c_i is too close to 0, set to +-eps. Reason: a zero unit can be trivially matched with zero weights.
        # self.C[(abs(self.C) < 0.1) & (self.C < 0)] = -0.1
        # self.C[(abs(self.C) < 0.1) & (self.C > 0)] = 0.1
...
def efficientnet_b4(pretrained=False, progress=True, num_classes=30, norm_layer=None, **kwargs):
