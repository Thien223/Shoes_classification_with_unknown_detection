...
from effi import efficientnet_b1, preprocess, efficientnet_b3
import time
...
if __name__ == '__main__':
	...
	from thien_inception import init_C
	##### load model:
	model = efficientnet_b1(pretrained=False, num_classes=80).to(device)
	model.to(device)
	checkpoint = torch.load('./checkpoints/' + 'best_model_b1' + '.pth', map_location=device)
	...
	C=init_C(model,train_loader,num_classes=len(classes))
	# C = torch.zeros(len(classes), device=device)
	R = torch.tensor(0, dtype=torch.float, device=device, requires_grad=True)
	svdd_scores_iod = val(val_loader=iod_loader, model=model, C=C, R=R, nu=0.1)
	svdd_scores_ood = val(val_loader=ood_loader, model=model, C=C, R=R, nu=0.1)


	result_path = f'svdd_scores/'
	os.makedirs(result_path, exist_ok=True)
	print(f'{svdd_scores_iod}')
	write_list_to_txt(svdd_scores_iod, os.path.join(result_path, "in_of_distribution.txt"))
