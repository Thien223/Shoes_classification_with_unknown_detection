...
def train(train_loader, model, optimizer, criterion):
    ...
    for i, sample in enumerate(train_loader):
        ...
        # svdd_loss = svdd_loss / 10
        svdd_loss = torch.sum(distance, dim=0)
        model_loss = criterion(class_out, target)
        loss = model_loss + svdd_loss
        # print(f'distance {svdd_loss}')
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, epoch_max):
        model = train(train_loader, model, optimizer, criterion1)
        svdd_scores_iod, iod_val_acc = val(val_loader=test_loader, model=model)
        svdd_scores_ood, _ = val(val_loader=ood_loader, model=model)

        result_path = os.path.join(result_path,f'epoch_{epoch}')
        ...
        # print(iod_val_acc)
        # print(ood_val_acc)
        total_acc = np.sqrt(((float(iod_val_acc.cpu().numpy())+float(ood_val_acc))/2)**2 - ((float(iod_val_acc.cpu().numpy())-float(ood_val_acc))/2)**2)

        if total_acc > best_acc:
            best_acc = total_acc
            save_model(model=model, optimizer=optimizer, epoch=epoch, path=checkpoint)

        save_model(model=model, optimizer=optimizer, epoch=epoch, path=os.path.join(checkpoints_folder,f'epoch-{epoch}_val_acc-{iod_val_acc}_ood_acc-{ood_val_acc}.pth'))
